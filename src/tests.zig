...
test "basic encrypt/decrypt roundtrip" {
    ...
    var recipient = try h.recipientSetup(sender.encapsulatedSecret(), &sk_r, "test info");
...
test "export secret" {
    ...
    var sender = try h.senderSetup(&pk_r, "test info", io);
    const recipient = try h.recipientSetup(sender.encapsulatedSecret(), &sk_r, "test info");
...
test "wrong key fails decryption" {
    ...
    io.random(&wrong_sk);
    var wrong_recipient = try h.recipientSetup(sender.encapsulatedSecret(), &wrong_sk, "test info");
...
test "test vectors" {
    ...
    const server_sk = server_kp.secret_key[0..32];
    const server_pk = server_kp.public_key[0..32];
    ...
    const client_sk_e = client_kp.secret_key[0..32];

    var sender = try h.senderSetupDeterministic(server_pk, &info, client_sk_e);
    const enc = sender.encapsulatedSecret();
...
test "RFC 9180 A.1: X25519/HKDF-SHA256/AES-128-GCM base mode" {
    ...
    try testing.expectEqualSlices(u8, &expected_sk_e, kp_e.secret_key[0..32]);
    ...
    try testing.expectEqualSlices(u8, &expected_pk_e, kp_e.public_key[0..32]);
    ...
    try testing.expectEqualSlices(u8, &expected_sk_r, kp_r.secret_key[0..32]);
    ...
    try testing.expectEqualSlices(u8, &expected_pk_r, kp_r.public_key[0..32]);

    var info: [20]u8 = undefined;
    _ = try hexToBytes(&info, "4f6465206f6e2061204772656369616e2055726e");

    var sender = try h.senderSetupDeterministic(kp_r.public_key[0..32], &info, kp_e.secret_key[0..32]);
    ...
    var recipient = try h.recipientSetup(sender.enc[0..32], kp_r.secret_key[0..32], &info);
...
test "RFC 9180 A.3: X25519/HKDF-SHA256/AES-128-GCM PSK mode" {
    ...
    var sender = try h.senderSetupDeterministicPSK(kp_r.public_key[0..32], &info, &psk, &psk_id, kp_e.secret_key[0..32]);
    ...
    var recipient = try h.recipientSetupPSK(sender.enc[0..32], kp_r.secret_key[0..32], &info, &psk, &psk_id);
    var decrypted: [29]u8 = undefined;
    try recipient.open(&decrypted, &ct, &aad);
    try testing.expectEqualSlices(u8, &pt, &decrypted);
...
test "RFC 9180 A.1.3: X25519/HKDF-SHA256/AES-128-GCM auth mode" {
    ...
    try testing.expectEqualSlices(u8, &expected_sk_s, kp_s.secret_key[0..32]);
    ...
    try testing.expectEqualSlices(u8, &expected_pk_s, kp_s.public_key[0..32]);

    var info: [20]u8 = undefined;
    _ = try hexToBytes(&info, "4f6465206f6e2061204772656369616e2055726e");

    var sender = try h.senderSetupDeterministicAuth(kp_r.public_key[0..32], &info, kp_s.secret_key[0..32], kp_e.secret_key[0..32]);
    var recipient = try h.recipientSetupAuth(sender.enc[0..32], kp_r.secret_key[0..32], &info, kp_s.public_key[0..32]);
...
test "RFC 9180 A.1.4: X25519/HKDF-SHA256/AES-128-GCM auth+psk mode" {
    ...
    var sender = try h.senderSetupDeterministicAuthPSK(kp_r.public_key[0..32], &info, &psk, &psk_id, kp_s.secret_key[0..32], kp_e.secret_key[0..32]);
    var recipient = try h.recipientSetupAuthPSK(sender.enc[0..32], kp_r.secret_key[0..32], &info, &psk, &psk_id, kp_s.public_key[0..32]);
...
test "RFC 9180 A.6: P-256/HKDF-SHA256/AES-128-GCM base mode" {
    ...
    try testing.expectEqualSlices(u8, &expected_pk_e, kp_e.public_key[0..65]);
    ...
    try testing.expectEqualSlices(u8, &expected_pk_r, kp_r.public_key[0..65]);

    var info: [20]u8 = undefined;
    _ = try hexToBytes(&info, "4f6465206f6e2061204772656369616e2055726e");

    var sender = try h.senderSetupDeterministic(kp_r.public_key[0..65], &info, kp_e.secret_key[0..32]);
    ...
    var recipient = try h.recipientSetup(sender.enc[0..65], kp_r.secret_key[0..32], &info);
    var decrypted: [29]u8 = undefined;
    try recipient.open(&decrypted, &ct, &aad);
    try testing.expectEqualSlices(u8, &pt, &decrypted);
...
test "setup and export accept long RFC-valid inputs" {
    ...
    var sender = try h.senderSetup(kp_r.public_key[0..32], &info, io);
    var recipient = try h.recipientSetup(sender.encapsulatedSecret(), kp_r.secret_key[0..32], &info);
...
test "PQ KEM roundtrip: ML-KEM-768" {
    ...
    const pk_r = kp_r.public_key[0..1184];
    const sk_r = kp_r.secret_key[0..64];

    var sender = try h.senderSetup(pk_r, "pq test", io);
    var recipient = try h.recipientSetup(sender.encapsulatedSecret(), sk_r, "pq test");
...
test "PQ KEM roundtrip: MLKEM768-X25519 (X-Wing)" {
    ...
    const pk_r = kp_r.public_key[0..1216];
    const sk_r = kp_r.secret_key[0..32];

    var sender = try h.senderSetup(pk_r, "xwing test", io);
    var recipient = try h.recipientSetup(sender.encapsulatedSecret(), sk_r, "xwing test");
...
test "PQ KEM roundtrip: MLKEM768-P256" {
    ...
    const pk_r = kp_r.public_key[0..1249];
    const sk_r = kp_r.secret_key[0..32];

    var sender = try h.senderSetup(pk_r, "hybrid p256 test", io);
    var recipient = try h.recipientSetup(sender.encapsulatedSecret(), sk_r, "hybrid p256 test");
...
test "PQ KEM rejects auth mode" {
    ...
    try testing.expectError(
        ...
        h.senderSetupAuth(kp_r.public_key[0..1184], "info", kp_s.secret_key[0..64], io),
    );
}
...
test "PQ KEM export secret" {
    ...
    var sender = try h.senderSetup(kp_r.public_key[0..1184], "export test", io);
    const recipient = try h.recipientSetup(sender.encapsulatedSecret(), kp_r.secret_key[0..64], "export test");
...
test "draft-ietf-hpke-pq-04 vector 0: ML-KEM-512 / HKDF-SHA256 / AES-128-GCM base mode" {
    ...
    try testing.expectEqualSlices(u8, &expected_skRm, kp_r.secret_key[0..64]);
    ...
    try testing.expectEqualSlices(u8, &expected_pkRm, kp_r.public_key[0..800]);
    ...
    var sender = try h.senderSetupDeterministic(kp_r.public_key[0..800], &info, &ikmE);
    ...
    try testing.expectEqualSlices(u8, &expected_enc, sender.encapsulatedSecret());
...
test "draft-ietf-hpke-pq-04 vector 1: ML-KEM-768 / HKDF-SHA256 / AES-128-GCM base mode" {
    ...
    var sender = try h.senderSetupDeterministic(kp_r.public_key[0..1184], &info, &ikmE);
...
test "draft-ietf-hpke-pq-04 vector 4: MLKEM768-X25519 / HKDF-SHA256 / ChaCha20Poly1305 base mode" {
    ...
    try testing.expectEqualSlices(u8, &expected_skRm, kp_r.secret_key[0..32]);
    ...
    var sender = try h.senderSetupDeterministic(kp_r.public_key[0..1216], &info, &ikmE);
...
test "draft-ietf-hpke-pq-04 vector 3: MLKEM768-P256 / HKDF-SHA256 / AES-128-GCM base mode" {
    ...
    var sender = try h.senderSetupDeterministic(kp_r.public_key[0..1249], &info, &ikmE);
...
test "draft-ietf-hpke-pq-04 vector 2: ML-KEM-1024 / HKDF-SHA384 / AES-256-GCM base mode" {
    ...
    var sender = try h.senderSetupDeterministic(kp_r.public_key[0..1568], &info, &ikmE);
...
test "draft-ietf-hpke-pq-04 vector 5: MLKEM1024-P384 / HKDF-SHA384 / AES-256-GCM base mode" {
    ...
    var sender = try h.senderSetupDeterministic(kp_r.public_key[0..1665], &info, &ikmE);
...
test "DHKEM + one-stage KDF PSK roundtrip" {
    ...
    var sender = try h.senderSetupPSK(kp_r.public_key[0..32], "psk test", psk, psk_id, io);
    var recipient = try h.recipientSetupPSK(sender.encapsulatedSecret(), kp_r.secret_key[0..32], "psk test", psk, psk_id);
